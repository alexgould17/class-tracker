...
class School:
    ...
    Attributes:
        identifier: A string (unenforced length) that uniquely identifies the school. For example: UIUC
        short_name: A short string (truncated to 15 chars) representing the School name. For example: Illinois
        pretty_name: A string representing the full, long name of a School. For example: University of Illinois at Urbana-Champaign
        grade_pts: A dictionary mapping strings representing grades to the number of grade points each is worth as a float. See the module-level global default for an example. Individual grade points can be added or removed from the dict using the helper methods.

    Raises:
        ValueError on any attribute assignment (including during initialization) that violates the above conditions.
        AlreadyExistsError when trying to add a School with an identifier that matches that of an existing School instance.
    """
    ...
    def identifier(self, value: str):
        ...
        elif type(value) is not str or value == '':
